                    ...
                    <div class="box">
                        <div class="row no-gutters" style="border-bottom: 3px #fff solid;">
                            <div class="col text-left">
                                <div class="add-new">
                                    <a href="{{url('attendance/create')}}">Add New<i class="fas fa-plus"></i></a>
                                </div><!-- End add-new-->
                            </div><!-- End column-->
                            <div class="col">
                                <div class="heading-messages">
                                    <h3 class="text-center">Attendance Listing</h3>
                                ...
                                @if(count($dataRow))
                                    @foreach($dataRow as $row)  
                                        <tr><td class="text-center">{{$row->id}}</td>
                                            ...
                                            <td class="text-center">{{$row->locker}}</td>
                                            <td class="text-center">{{$row->intime}}</td>
                                            <td class="text-center">{{$row->outtime}}</td>
                                            <td>{{formatDate($row->created_at)}}</td>
                                            <td>
                                                <a href="{{url('attendance/edit/'.$row->id)}}"><i class="fas fa-edit"></i></a>
                                                <a href="{{url('attendance/delete/'.$row->id)}}"><i class="fas fa-trash-alt"></i></a>
                                            </td>
